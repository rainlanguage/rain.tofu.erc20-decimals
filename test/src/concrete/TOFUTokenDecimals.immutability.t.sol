...
} from "rain.extrospection/lib/LibExtrospectBytecode.sol";
import {
    EVM_OP_SELFDESTRUCT,
    EVM_OP_DELEGATECALL,
    EVM_OP_CALLCODE
} from "rain.extrospection/lib/EVMOpcodes.sol";
...
contract TOFUTokenDecimalsImmutabilityTest is Test {
    ...
    function testNoMutableOpcodes() external {
        ...
        uint256 reachable = LibExtrospectBytecode
            .scanEVMOpcodesReachableInBytecode(bytecode);

        // SELFDESTRUCT would allow the contract to be destroyed.
        assertEq(
            reachable & (1 << EVM_OP_SELFDESTRUCT),
            0,
        ...
        // storage context.
        assertEq(
            reachable & (1 << EVM_OP_DELEGATECALL),
            0,
            "DELEGATECALL is reachable"
        );
        // CALLCODE would allow arbitrary code execution in the contract's
        // storage context.
        assertEq(
            reachable & (1 << EVM_OP_CALLCODE),
            0,
            "CALLCODE is reachable"
        );
    }
}
