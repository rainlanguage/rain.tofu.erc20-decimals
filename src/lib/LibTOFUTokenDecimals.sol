...
library LibTOFUTokenDecimals {
    ...
    /// @param expectedAddress The address where the singleton was expected.
    ...
    /// @notice Ensures that the TOFUTokenDecimals contract is deployed at the
    /// expected address with the expected codehash. Reverts with
    /// `TOFUTokenDecimalsNotDeployed` if the address has no code or the
    /// codehash does not match, preventing silent call failures.
